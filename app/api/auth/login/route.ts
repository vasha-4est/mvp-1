...
import { findUserByUsername, getRolesForUser, touchLastLoginAt } from "@/lib/server/controlModel";
...
export async function POST(request: Request) {
  ...
  if (!user) {
    return json(requestId, 401, { ok: false, code: "INVALID_CREDENTIALS" });
  }
  ...
  if (!user.is_active) {
    return json(requestId, 403, { ok: false, code: "ACCOUNT_INACTIVE" });
  }
