...
import { getShipmentsSlaKpi } from "@/lib/kpi/readShipmentsSla";
...
function parsePositiveMinutes(raw: string | null): { ok: true; value: number } | { ok: false; code: "BAD_REQUEST"; error: string } {
  if (!raw) return { ok: true, value: DEFAULT_OVERDUE_THRESHOLD_MINUTES };
  const parsed = Number(raw);
  if (!Number.isInteger(parsed) || parsed < 1) {
    return { ok: false, code: "BAD_REQUEST", error: "Query param 'overdue_threshold_minutes' must be a positive integer" };
  }
...
export async function GET(request: Request) {
  ...
  const thresholdResult = parsePositiveMinutes(searchParams.get("overdue_threshold_minutes"));
  if (thresholdResult.ok === false) {
    return json(auth.requestId, 400, { ok: false, code: thresholdResult.code, error: thresholdResult.error });
  }

  const result = await getShipmentsSlaKpi(auth.requestId, {
    days: daysResult.value,
    tz: (searchParams.get("tz") || "").trim() || DEFAULT_TZ,
    overdue_threshold_minutes: thresholdResult.value,
  });
